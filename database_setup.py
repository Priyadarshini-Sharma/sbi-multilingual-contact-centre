...
SPECIALTIES = ["Fraud", "Loans", "General", "Video Advisory"]
...
def create_schema(conn):
    ...
    cur.execute("""
        CREATE TABLE Agents (
            ...
            specialties    TEXT NOT NULL,   -- comma-separated, e.g. "Loans,Video Advisory"
...
def seed_agents(conn):
    """8 agents covering the four active specialties (Fraud, Loans, General,
    Video Advisory), matched against the caller's detected language and
    weighted by current workload in the routing formula (optimizer.py).
    """
    agents = [
        # name,      languages,               specialties,                  workload, csat
        ("Rohan",    "Marathi,Hindi,English", "Fraud",                      0.35, 4.6),
        ("Priya",    "Tamil,English",         "Loans",                      0.20, 4.8),
        ...
        ("Ananya",   "Hindi,English,Punjabi", "General",                    0.25, 4.7),
        ("Vikram",   "English,Hindi,Malayalam","Fraud",                     0.40, 4.5),
