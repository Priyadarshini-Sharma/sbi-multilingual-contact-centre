...
2. **Classifies intent** across 7 service categories, including 3 new value-added services (Proactive Fraud Lock, Video Banking Advisory, Passive Voice Biometrics)
...
- **First-Call Resolution:** 38.7% (Traditional IVR) → **79.5%** (this system)
- **Churn risk rate:** 16.4% → **3.7%**
- **Average sentiment improvement:** +0.52 → **+0.63**

Manual validation against a 25-case test set spanning 11 languages and 7 intent categories showed strong accuracy on intent classification and routing (23/25, 92%), with sentiment analysis and language detection as the primary areas for future refinement (see Limitations).
...
- **Language auto-detection on some native scripts:** In the 25-case manual test suite, one native-script Malayalam transcript was auto-detected as Marathi by the translation layer, though the correct intent was still extracted downstream. This did not affect routing correctness in that case but is a documented gap in language-detection reliability distinct from the romanized-input issue above.
